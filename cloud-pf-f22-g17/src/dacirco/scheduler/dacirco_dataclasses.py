...
class PodParameters:
    ...
    image: str  # The name of the image to use
    cpus: int  # The number of vCPUs of each pod
    memory: str  # The maximum memory for each pod
    node_selector: str  # The name of the node where pods must run
    concentrate_pods: bool  # If True, put all pods on the same node
    spread_pods: bool  # If True, spread pods on different nodes
