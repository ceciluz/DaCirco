...
class PodManager(RunnerManager):
    ...
    def create_runner(self) -> RunnerManagerRes:
        if self._pod_count == self._MAX_WORKERS:
            ...
            pod_affinity = {}
            if self._pod_parameters.spread_pods:
                pod_affinity = {
                    "podAntiAffinity": {
            ...
            if self._pod_parameters.concentrate_pods:
                pod_affinity = {
                    "podAffinity": {
                        "requiredDuringSchedulingIgnoredDuringExecution": [
                            {
            ...
            limits = {}
            if self._pod_parameters.cpus != 0:
                limits["cpu"] = str(self._pod_parameters.cpus)
            if self._pod_parameters.memory != "0":
                limits["memory"] = self._pod_parameters.memory
            pod_manifest = {
                "apiVersion": "v1",
                "kind": "Pod",
    ...
    def destroy_idle_workers(self, threshold: timedelta):
        targets: list[str] = []
        print("Timer - Runner Count :", self._pod_count, "|", self._MAX_WORKERS)
    ...
    def get_runner_info(self, worker_id: str) -> tuple[str, int, str]:
        ...
        if worker:
            ...
            if "cpu" in ret.spec.containers[0].resources.limits.keys():
                cpus = ret.spec.containers[0].resources.limits["cpu"]
            if "memory" in ret.spec.containers[0].resources.limits.keys():
                memory = ret.spec.containers[0].resources.limits["memory"]
