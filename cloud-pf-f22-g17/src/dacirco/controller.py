...
@click.option(
    ...
    default=2,
    show_default=True,
    help="The number of vCPUs of each pod",
)
@click.option(
    "--pod-memory",
    default="1.4Gi",
...
@click.option(
    "--concentrate-pods",
    is_flag=True,
    help="[EXPERT] Place all the pods on the same node",
)
@click.option(
    "--spread-pods",
    is_flag=True,
    help="[EXPERT] Place pods on different nodes",
)
def run(
    verbose: int,
    port: int,
    ...
    pod_parameters = PodParameters(
        ...
        concentrate_pods=concentrate_pods,
